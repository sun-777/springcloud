 ...
 *               相比基类中自动注入HttpServletRequest、HttpServletResponse，
 *               BaseController可以允许子类实现此接口获得HttpServletRequest、HttpServletResponse；同时子类还可以继承其它需要的类。
 * @author: Sun Xiaodong
 */
        ...
        if (enumValues.length > 0 && !StringUtil.isBlank(enumString)) {
            ...
            try {
                ...
                if (Integer.class.isInstance(c)) {  // C是Integer类型
                    ...
                    final Long code = Long.valueOf(enumString);
                    enumObject = (T) enumValues[0].codeOf((C) code).orElse(null);
                } else {
